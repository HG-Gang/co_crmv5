...
/**
 * 后台资金运营六个小型控制器十条旧路由的权威矩阵门禁。
 *
 * 文件功能：
 * - 断言 admin_funding_ops_misc_business_2026_08_21 证据组覆盖全部十条方法级路由。
 * - 每条路由必须 verified、绑定正确当前路由名，且七个核验维度全部 passed。
 * - 每个维度的证据文本必须包含对应路由键，保证方法级说明不共享混写。
 * - 全局 summary 必须从 475 行重算且 verified 不低于本批次下限。
 */
final class AdminFundingOpsMiscBusinessMatrixClosureTest extends TestCase
{
    /**
     * 资金运营杂项（流水/汇率/在线用户等小型控制器）证据组的键名。按它从核验矩阵 JSON
     * （storage/app/audits/旧项目模块逻辑迁移核验矩阵.json）筛选本模块行并断言
     * 每行 verification_group 与之一致；值不匹配即门禁失败，防止证据混组。
     */
    private const VERIFICATION_GROUP = 'admin_funding_ops_misc_business_2026_08_21';
    ...
    /**
     * 资金运营杂项模块的旧路由认领清单（十条）：键为「HTTP 方法 旧URI」，值为矩阵认定的当前路由名。
     * 门禁逐条断言 verified、绑定与七维独立出证；路由增删必须同步维护，否则漏出证或误报。
     */
    private const EXPECTED_ROUTES = [
        'GET index/admin/amount/depositDownloadfile/{file}/{role}' => 'legacy_admin_d4a1c209539dac9a',
        'GET index/admin/amount/deposit_flow' => 'legacy_admin_c59f22232bf2630a',
        'GET index/admin/amount/show_channel_browse' => 'legacy_admin_82597987e49fc242',
        'GET index/admin/amount/undeposit_flow' => 'legacy_admin_debfbf74887834f4',
        'GET index/admin/amount/whpj_rate' => 'legacy_admin_8258c344996d12ad',
        'POST index/admin/amount/whpj_rate_save' => 'legacy_admin_cdbae847cf109322',
        'GET index/admin/amount/withdrawDownloadfile/{file}/{role}' => 'legacy_admin_a836d3f3c6d032fc',
        'GET index/admin/amount/withdraw_flow' => 'legacy_admin_3474117a258d4cc6',
        'GET index/admin/online' => 'legacy_admin_505c109b749882b5',
        'POST index/admin/online/search' => 'legacy_admin_5d9775deb3f11717',
    ];

    public function test_all_ten_funding_ops_routes_have_independent_complete_evidence(): void
    {
    ...
    public function test_global_matrix_summary_reflects_the_funding_ops_closure(): void
    {
        $matrix = $this->readMatrix();
        $counts = [
            'verified' => 0,
        ...
        $this->assertGreaterThanOrEqual(476, $counts['verified']);
        $this->assertSame(476, $counts['verified']);
        $this->assertSame(0, $counts['needs_manual_business_review']);
        $this->assertSame(0, $counts['unresolved_legacy_source']);
        $this->assertSame(0, $counts['unmatched_current_route']);
    }
