...
/**
 * 创建用户业务资料表 user_infos。
 *
 * 文件功能：
 * - 前台用户核心业务档案：身份层级、资金账户（MT4 快照）、认证状态、结算与风控配置。
 *
 * 字段语义：
 * - user_id/login_id 业务与登录 ID；user_name/phone 用户名与电话；gender 性别；
 * - avatar 头像；level_id/group_id 代理级别与交易组；parent_id 上级代理；
 * - account_type 账户类型（1=代理 2=客户）；family_tree 逗号分隔祖先链；
 * - total_funds/used_margin/avail_margin/equity 等 MT4 资金快照字段（两位小数）；
 * - is_ecn/follow_parent_ecn ECN 模式；auth_status 实名认证状态（0=未验证 1=已验证 2=已退回 3=已禁用）；
 * - is_mt4_synced/enabled/readonly MT4 同步与权限；is_withdrawal_allowed/is_deposit_allowed 出入金开关；
 * - mt4_group/mt4_code MT4 分组与编号；trading_mode 交易模式（0=佣金 1=净值）；
 * - settle_method 结算方式（1=线上 2=线下）；settle_cycle 结算周期（1=每周 2=每两周 3=每月）；
 * - 时间字段均为 10 位时间戳。
 */
