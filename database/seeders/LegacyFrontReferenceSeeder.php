 ...
 *
 * 文件功能：
 * - 从 hank_zl_data 读取等级、交易组、品种、点差、系统参数、支付通道和新闻。
 * - 交易组通过 LegacyGroupConfigSynchronizer 分两阶段迁移旧身份与当前配对关系。
 * - 所有写入在当前数据库事务中完成，任一参考数据失败会回滚本次导入。
 */
class LegacyFrontReferenceSeeder extends Seeder
{
    ...
    /**
     * 提现必需系统配置键全集（开关/时段/限额/风控阈值/费率/汇率）。参考数据导入完成前先确保这些键存在，
     * 缺少任何一键前台提现链路都会因配置缺失而失败关闭；清单与 WritesRequiredWithdrawalConfigs 保持同步。
     *
     * @var array<int, string>
     */
    private const REQUIRED_WITHDRAWAL_CONFIG_KEYS = [
        'withdrawal_enabled',
    ...
    /**
     * 旧库（hank_zl_data）连接实例。run() 前由 connectLegacy() 建立，等级/交易组/品种等
     * 参考数据以它为只读源；写入全部走默认连接并在事务内完成。
     *
    ...
    /**
     * 各参考数据模块的导入计数（模块名 => 条数）。run() 末尾输出供验收核对；
     * 初始为空，模块未产出数据时表现为缺失行。
     *
    ...
    /**
     * 同步旧 group_config，并修复用户所属组主键。
     *
     * @param int $now 缺少有效旧时间时使用的当前 Unix 时间戳。
     * @return void
     */
    private function seedGroupConfigs(int $now): void
    {
        $rows = $this->legacy->table('group_config')->orderBy('id')->get();
        $groups = $rows->map(function ($row) use ($now) {
            return [
                'legacy_group_id' => (int) $row->id,
                'legacy_pair_id' => (int) ($row->pair_id ?? 0) ?: null,
        ...
        $mapping = app(LegacyGroupConfigSynchronizer::class)->synchronize($groups, $now, true);
        $this->counts['group_configs'] = count($mapping);
    }
    ...
    private function seedSystemConfigs(int $now): void
    {
        ...
        foreach ($system as $key => $value) {
            ...
            if (in_array($key, self::REQUIRED_WITHDRAWAL_CONFIG_KEYS, true)) {
                continue;
            }
            $this->putConfig($key, $value, 'legacy_system', 'Imported from hank_zl_data.system_config.' . $key, $now);
        }

        $hasSystemConfig = !empty($system);
        $this->putRequiredWithdrawalConfig('withdraw_exchange_rate_cny', $system['sys_draw_rate'] ?? '6.8', 'finance', 'Legacy withdrawal CNY rate', $now, $hasSystemConfig);
        $this->putRequiredWithdrawalConfig('withdraw_risk_rate_limit', $system['sys_draw_risk'] ?? '100', 'finance', 'Legacy withdrawal risk-rate limit', $now, $hasSystemConfig);
        $this->putRequiredWithdrawalConfig('withdrawal_fixed_fee_usd', $system['sys_poundage_money'] ?? '0', 'finance', 'Legacy fixed withdrawal fee', $now, $hasSystemConfig);
        $this->putRequiredWithdrawalConfig('withdrawal_fee_rate', '0', 'finance', 'Legacy non-OTC withdrawal fee rate', $now, $hasSystemConfig);
    ...
    private function mapGlobalRules($params, int $now): void
    {
        ...
        $withdrawGlobalValue = $this->validWithdrawalGlobalValue($withdrawGlobal);
        $withdrawRange = $this->validWithdrawalRuleRange($withdrawRule);
        ...
        $this->putRequiredWithdrawalConfig(
            'withdrawal_enabled',
        ...
        $this->putRequiredWithdrawalConfig('withdrawal_start_time', $withdrawStart, 'finance', 'Mapped from WITHDRAWRULE weekday range', $now, $hasWithdrawRule);
        $this->putRequiredWithdrawalConfig('withdrawal_end_time', $withdrawEnd, 'finance', 'Mapped from WITHDRAWRULE weekday range', $now, $hasWithdrawRule);
        ...
        $this->putRequiredWithdrawalConfig(
            'withdrawal_weekend_enabled',
            $hasWithdrawRule && $this->ruleAllowsWeekend($withdrawRule) ? '1' : '0',
            'finance',
        ...
        $this->putRequiredWithdrawalConfig(
            ...
            '1',
            'finance',
            'Legacy safe default open-position withdrawal check',
            $now,
            false
        );
    }
    ...
    private function mapAmountLimits($params, int $now): void
    {
        ...
        $hasWithdrawalRules = $this->validWithdrawalGlobalValue($params->get('GLOBALWITHDRAWRULE')) !== null
            || $this->validWithdrawalRuleRange($params->get('WITHDRAWRULE')) !== null;
        $this->putRequiredWithdrawalConfig('withdraw_min_amount', '300', 'finance', 'Legacy withdrawal page minimum amount', $now, $hasWithdrawalRules);
        $this->putRequiredWithdrawalConfig('withdraw_max_amount', '500000', 'finance', 'Legacy withdrawal maximum amount fallback', $now, $hasWithdrawalRules);
    }
    ...
    private function seedPaymentChannels(int $now): void
    {
        ...
        foreach ($params as $row) {
            ...
            $config = [
                ...
                'remark_items' => $this->legacyPaymentChannelRemarkItems($id),
    ...
    private function putRequiredWithdrawalConfig(
        string $key,
        $value,
        string $group,
        string $description,
    ...
    private function validWithdrawalGlobalValue($row): ?string
    {
        if (!$row) {
            return null;
        }

        $value = trim((string) ($row->para_data0 ?? ''));

        return in_array($value, ['0', '1'], true) ? $value : null;
    }

    /** @return array{0: string, 1: string}|null */
    private function validWithdrawalRuleRange($row): ?array
    {
        if (!$row) {
            return null;
        }

        $range = trim((string) ($row->para_data1 ?? ''));
        if ($range === '') {
            $range = trim((string) ($row->para_data0 ?? ''));
        }
        if ($range === '') {
            return null;
        }
        ...
        $start = $this->normalizeWithdrawalTime($parts[0]);
        $end = $this->normalizeWithdrawalTime($parts[1]);
    ...
    private function normalizeWithdrawalTime(string $value): ?string
    {
    ...
    private function legacyPaymentChannelRemarkItems(int $id): array
    {
        $map = [
            1 => [
                'Minimum transaction limit per trade: $500.',
                'Daily single-card transactions: Unlimited.',
                'Maximum transaction limit per trade: $6,800 (as set by banks).',
                'Cardholder transaction deduction currency: CNY.',
            ],
            2 => [
                'Minimum transaction limit per trade: $1,000.',
                'Daily single-card transactions: Unlimited.',
                'Maximum transaction limit per trade: $30,000 (as set by banks).',
                'Cardholder transaction deduction currency: CNY.',
            ],
            3 => [
                'Minimum transaction limit per trade: $500.',
                'Daily single-card transactions: Unlimited.',
                'Maximum transaction limit per trade: $80,000 (as set by banks).',
                'Cardholder transaction deduction currency: CNY.',
            ],
            4 => [
                '1 USD = 1 USDT.',
                'USDT transaction minimum: 500 USDT.',
                'Daily single-card transactions: Unlimited.',
            ...
            8 => [
                'UnionPay single transaction: $500-$14,000.',
                'Alipay single transaction: $500-$2,800.',
                'Daily single-card transactions: Unlimited.',
                'Cardholder transaction deduction currency: CNY.',
            ],
            9 => [
                'Single transaction: $500-$80,000.',
                'Daily single-card transactions: Unlimited.',
                'Cardholder transaction deduction currency: CNY.',
            ],
            10 => [
