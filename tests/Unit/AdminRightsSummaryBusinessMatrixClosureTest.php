 ...
 *
 * 文件功能：
 * - 断言 admin_rights_summary_business_2026_08_21 证据组覆盖全部五条方法级路由。
 * - 每条路由必须 verified、绑定正确当前路由名，且七个核验维度全部 passed。
 * - 每个维度的证据文本必须包含对应路由键，保证方法级说明不共享混写。
 * - 全局 summary 必须从 475 行重算且 verified 不低于权益汇总批次下限。
 */
final class AdminRightsSummaryBusinessMatrixClosureTest extends TestCase
{
    /**
     * 权益汇总证据组的键名。按它从核验矩阵 JSON（storage/app/audits/旧项目模块逻辑迁移核验矩阵.json）
     * 筛选本模块行并断言每行 verification_group 与之一致；值不匹配即门禁失败，防止证据混组。
     */
    private const VERIFICATION_GROUP = 'admin_rights_summary_business_2026_08_21';
    ...
    /**
     * 权益汇总模块的旧路由认领清单（五条）：键为「HTTP 方法 旧URI」，值为矩阵认定的当前路由名。
     * 门禁逐条断言 verified、绑定与七维独立出证；路由增删必须同步维护，否则漏出证或误报。
     */
    private const EXPECTED_ROUTES = [
        'POST index/admin/amount/rightsSumExport' => 'legacy_admin_2998b4026312854e',
        'POST index/admin/amount/rightsSummarySearch' => 'legacy_admin_8126edd88d7bf122',
        'GET index/admin/amount/rightsSummarySearchDetail/{uid}/{status}/{sumdata}' => 'legacy_admin_e43fc2ab54c96fb4',
        'GET index/admin/amount/rights_downloadfile/{file}/{role}' => 'legacy_admin_11e05db1e4be5c77',
        'GET index/admin/amount/rights_summary' => 'legacy_admin_eaa47e867df9a078',
    ];

    public function test_all_five_rights_summary_routes_have_independent_complete_evidence(): void
    {
    ...
    public function test_global_matrix_summary_reflects_the_rights_summary_closure(): void
    {
        $matrix = $this->readMatrix();
        $counts = [
            'verified' => 0,
        ...
        $this->assertGreaterThanOrEqual(450, $counts['verified']);
    }
