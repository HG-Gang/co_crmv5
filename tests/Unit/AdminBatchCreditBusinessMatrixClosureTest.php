...
/**
 * 后台批量授信导入三条旧路由的权威矩阵门禁。
 *
 * 文件功能：
 * - 断言 admin_batch_credit_business_2026_08_21 证据组覆盖全部三条方法级路由。
 * - 每条路由必须 verified、绑定正确当前路由名，且七个核验维度全部 passed。
 * - 每个维度的证据文本必须包含对应路由键，保证方法级说明不共享混写。
 * - 全局 summary 必须从 475 行重算且 verified 不低于本批次下限。
 */
final class AdminBatchCreditBusinessMatrixClosureTest extends TestCase
{
    /**
     * 批量授信导入证据组的键名。按它从核验矩阵 JSON（storage/app/audits/旧项目模块逻辑迁移核验矩阵.json）
     * 筛选本模块行并断言每行 verification_group 与之一致；值不匹配即门禁失败，防止证据混组。
     */
    private const VERIFICATION_GROUP = 'admin_batch_credit_business_2026_08_21';
    ...
    /**
     * 批量授信导入的旧路由认领清单：键为「HTTP 方法 旧URI」，值为矩阵认定的当前路由名。
     * 门禁逐条断言 verified、绑定与七维独立出证；路由增删必须同步维护，否则漏出证或误报。
     */
    private const EXPECTED_ROUTES = [
        'POST index/admin/credit/creditImportExcel' => 'legacy_admin_defded728918bca3',
        'POST index/admin/credit/creditImportSearch' => 'legacy_admin_0dac2b9fe002ca51',
        'GET index/admin/credit/credit_import_index' => 'legacy_admin_0db90f874c729f1f',
    ];

    public function test_all_three_batch_credit_routes_have_independent_complete_evidence(): void
    {
        $matrix = $this->readMatrix();
        $hashByName = [];
        foreach ($matrix['rows'] as $row) {
            if ((string) ($row['legacy_action'] ?? '') === 'App\\Http\\Controllers\\Admin\\LegacyAdminController@handle'
                && strpos((string) $row['legacy_uri'], 'index/admin/credit/') === 0) {
                $hashByName[ltrim((string) $row['legacy_uri'], '/')] = (string) $row['current_name'];
            }
        }
        ...
        foreach (self::EXPECTED_ROUTES as $key => $fallbackName) {
            $row = $rows[$key];
            $uri = ltrim((string) $row['legacy_uri'], '/');
            $expectedName = $hashByName[$uri] ?? $fallbackName;
            $this->assertSame('verified', $row['evidence_state'], $key);
            $this->assertSame(self::VERIFICATION_GROUP, $row['verification_group'], $key);
            $this->assertSame($expectedName, $row['current_name'], $key);

            $verification = $row['verification'];
            $this->assertSame('verified', $verification['state'], $key);
            $this->assertSame($expectedName, $verification['current_route']['name'], $key);
            $this->assertSame(
    ...
    public function test_global_matrix_summary_reflects_the_batch_credit_closure(): void
    {
        $matrix = $this->readMatrix();
        $counts = [
            'verified' => 0,
        ...
        $this->assertGreaterThanOrEqual(462, $counts['verified']);
    }
