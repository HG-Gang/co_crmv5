 ...
 *
 * 文件功能：
 * - 断言 admin_cancellation_business_2026_08_19 证据组覆盖全部六条方法级路由。
 * - 每条路由必须 verified、绑定正确当前路由名，且七个核验维度全部 passed。
 * - 每个维度的证据文本必须包含对应路由键，保证方法级说明不共享混写。
 * - 全局 summary 必须反映注销批次后的 440/35 完成度。
 */
final class AdminCancellationBusinessMatrixClosureTest extends TestCase
{
    /**
     * 注销申请证据组的键名。按它从核验矩阵 JSON（storage/app/audits/旧项目模块逻辑迁移核验矩阵.json）
     * 筛选本模块行并断言每行 verification_group 与之一致；值不匹配即门禁失败，防止证据混组。
     */
    private const VERIFICATION_GROUP = 'admin_cancellation_business_2026_08_19';
    ...
    /**
     * 注销申请模块的旧路由认领清单：键为「HTTP 方法 旧URI」，值为矩阵认定的当前路由名。
     * 门禁逐条断言 verified、绑定与七维独立出证；路由增删必须同步维护，否则漏出证或误报。
     */
    private const EXPECTED_ROUTES = [
        'POST index/admin/cancel/cancel_apply_nopass' => 'legacy_admin_780daad6e1036413',
        'POST index/admin/cancel/cancel_apply_pass' => 'legacy_admin_38bdd209ea69d760',
        'POST index/admin/cancel/update_cancel' => 'legacy_admin_b2cd41c5659ac6cd',
        'GET index/admin/cancel/user_list' => 'legacy_admin_b90af4eb78f3a96c',
        'POST index/admin/cancel/userlistSearch' => 'legacy_admin_bf4dba351ec38c73',
        'POST index/admin/cancel/userlistSearchV2' => 'legacy_admin_f4912de7d63fb76d',
    ];

    public function test_all_six_cancellation_routes_have_independent_complete_evidence(): void
    {
    ...
    public function test_global_matrix_summary_reflects_the_cancellation_closure(): void
    {
        $matrix = $this->readMatrix();
        $counts = [
            'verified' => 0,
        ...
        $this->assertGreaterThanOrEqual(440, $counts['verified']);
    }
