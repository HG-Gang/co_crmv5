 ...
 *
 * 文件功能：
 * - 断言 admin_production_business_2026_08_21 证据组覆盖全部三条方法级路由。
 * - 每条路由必须 verified、绑定正确当前路由名，且七个核验维度全部 passed。
 * - 每个维度的证据文本必须包含对应路由键，保证方法级说明不共享混写。
 * - 全局 summary 必须从 475 行重算且 verified 不低于本批次下限。
 */
final class AdminProductionBusinessMatrixClosureTest extends TestCase
{
    /**
     * 产量列表证据组的键名。按它从核验矩阵 JSON（storage/app/audits/旧项目模块逻辑迁移核验矩阵.json）
     * 筛选本模块行并断言每行 verification_group 与之一致；值不匹配即门禁失败，防止证据混组。
     */
    private const VERIFICATION_GROUP = 'admin_production_business_2026_08_21';
    ...
    /**
     * 产量列表模块的旧路由认领清单（三条）：键为「HTTP 方法 旧URI」，值为矩阵认定的当前路由名。
     * 门禁逐条断言 verified、绑定与七维独立出证；路由增删必须同步维护，否则漏出证或误报。
     */
    private const EXPECTED_ROUTES = [
        'POST index/admin/order/productionListSearch' => 'legacy_admin_b7e927aba7540fdb',
        'POST index/admin/order/productionListSearchV2' => 'legacy_admin_4feff3fd724221b2',
        'GET index/admin/order/production_list' => 'legacy_admin_7104bfc863644e14',
    ];

    public function test_all_three_production_routes_have_independent_complete_evidence(): void
    {
    ...
    public function test_global_matrix_summary_reflects_the_production_closure(): void
    {
        $matrix = $this->readMatrix();
        $counts = [
            'verified' => 0,
        ...
        $this->assertGreaterThanOrEqual(465, $counts['verified']);
    }
