 ...
 * 它们收拢到 Laravel/Layui 版本，所有页面都可以复用统一的响应整理、
...
var CrmTable = (function () {
    ...
    function isSuccess(res) {
        ...
        // 2xxx contains business-rule failures. Only documented success codes
        // may drive table rendering or mutation cleanup.
    ...
    /**
     * 将 Laravel 分页、{list,total}、纯数组和嵌套 list 返回统一成一个
     * 结构，方便静态 HTML 表格与 Layui 表格共用。
     */
    ...
    function isSummableColumn(column) {
        ...
        // 电话、地址等文本字段不参与自动汇总，避免手机号、门牌号被误当成金额或数量。
        if (/(phone|mobile|tel|address|recipient_phone|recipient_address)/.test(key)) {
            return false;
        }
    ...
    /**
     * 旧 common.js 里暴露过名为 "checkField" 的批量表格操作。
     ...
     * 这里复刻旧 modalBoxByPage 的行为，同时将 Layui 相关代码集中起来。
     */
