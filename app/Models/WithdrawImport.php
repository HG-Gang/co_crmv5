...
class WithdrawImport extends BaseModel
{
     ...
     * 模型绑定的数据表名称。
     *
     * @var string $table 表示当前模型读写的真实数据库表，固定为 withdraw_imports。
     */
     ...
     *
     * 逻辑说明：
     * - 保持重试/导入 API 的 JSON 载荷类型稳定：金额统一输出为两位小数字符串，
     *   用户 ID、订单号和状态字段统一转为整数，避免不同前端栈收到混合类型导致比较或展示异常。
     *
     ...
     *
     * 参数逻辑说明：
     * - 外键 user_id 来自 withdraw_imports.user_id，表示导入记录归属的业务用户。
