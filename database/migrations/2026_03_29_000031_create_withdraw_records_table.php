...
/**
 * 创建提现记录表 withdraw_records。
 *
 * 文件功能：
 * - 前台用户提现订单：申请/实付金额、手续费、收款银行卡、渠道订单与状态。
 *
 * 字段语义：
 * - user_id/user_name 用户；mt4_ticket MT4 订单号；apply_amount 申请金额；actual_amount 实付金额；
 * - fee/rmb_fee 手续费（外币/人民币）；exchange_rate 汇率；bank_no/bank_name/bank_addr 收款银行卡；
 * - status 状态（0=待处理 1=处理中 2=完成 3=失败）；local_order_no/third_order_no 本地/第三方订单号；
 * - reject_reason 拒绝原因；mt4_return_status MT4 返回状态；created_by/updated_by 操作人。
 */
