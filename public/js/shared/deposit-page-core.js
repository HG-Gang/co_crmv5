...
var DepositPageCore = (function () {
    ...
    function isSuccess(res) {
        return !!(res && Number(res.code) >= 1000 && Number(res.code) < 2000);
    }
    ...
    function init(options) {
        ...
        function createIdempotencyKey() {
            if (window.crypto && typeof window.crypto.randomUUID === 'function') {
                return 'dpt-' + window.crypto.randomUUID();
            }

            return 'dpt-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 14);
        }

        function renderedIntentKey() {
            var value = String($(opts.formSelector).find('[name="idempotency_key"]').val() || '').trim();

            return /^[a-f0-9]{64}$/.test(value) ? value : '';
        }

        function currentIdempotencyKey(amount, channel) {
            var rendered = renderedIntentKey();
            var fingerprint = String(amount) + '|' + String(channel || '');

            if (rendered) {
                return rendered;
            }
            if (!pageState.idempotencyKey || pageState.idempotencyFingerprint !== fingerprint) {
                pageState.idempotencyKey = createIdempotencyKey();
                pageState.idempotencyFingerprint = fingerprint;
            }

            return pageState.idempotencyKey;
        }

        function clearIdempotencyKey() {
            var $input = $(opts.formSelector).find('[name="idempotency_key"]');

            pageState.idempotencyKey = null;
            pageState.idempotencyFingerprint = null;
            if ($input.length) {
                $input.val('');
            }
        }
        ...
        function renderHistoryTable() {
            table.render(CrmTable.layuiConfig('front', {
                elem: opts.tableElem,
                url: opts.historyApi,
                method: opts.historyMethod || 'POST',
        ...
        function submitDeposit(field) {
            ...
            var amount = normalizeAmount(field.deposit_amt_usd || field.amount);
            ...
            if (channel.min_amount && numericAmount < Number(channel.min_amount)) {
                layer.msg(t('front.amount_below_channel_min'), {icon: 2});
                return;
            }
            if (channel.max_amount && numericAmount > Number(channel.max_amount)) {
                layer.msg(t('front.amount_above_channel_max'), {icon: 2});
                return;
            }

            requestKey = currentIdempotencyKey(amount, channel.code);
