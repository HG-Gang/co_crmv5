...
/**
 * 创建用户实名认证资料表 user_auths。
 *
 * 文件功能：
 * - 用户银行卡与身份证实名认证资料，含审核状态与变更中临时值。
 *
 * 字段语义：
 * - user_id 用户 ID；bank_no/bank_name/bank_addr 银行卡号、银行、分行地址；
 * - bank_card_img(_tmp) 银行卡图片（_tmp 为变更中临时图）；bank_status 银行卡状态
 *   （0=未通过 1=审核中 2=已通过 3=变更中 4=已拒绝）；bank_remarks 银行备注；
 * - id_card_no/id_card_front/id_card_back 身份证号与正反面照片；id_card_status 身份证状态
 *   （0=未通过 1=审核中 2=已通过 4=已退回）；id_card_remarks 身份证备注；
 * - is_bank_synced 银行信息是否已同步 MT4。
 */
