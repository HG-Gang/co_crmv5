 ...
 *
 * 文件功能：
 * - 从 hank_zl_data 读取用户、代理、资金、交易、凭证和层级业务数据。
 * - 把旧表字段转换为当前 Laravel 业务表结构，并保持用户身份与 MT4 状态一致。
 * - 迁移完成后恢复固定前台测试代理，供 Blade 页面和真实登录链路验收。
 *
 * 返回值：
 * - run 成功时完成全部数据写入并输出各模块计数。
 * - 旧库连接、字段映射或数据库约束失败时抛出异常，禁止伪造成功结果。
 */
class LegacyFrontBusinessDataSeeder extends Seeder
{
    /**
     * 旧库（hank_zl_data）连接实例。run() 前由 connectLegacy() 建立，全部业务数据以它为读取源；
     * 迁移过程中源数据只读，写入一律走默认连接，避免两个库写混。
     *
    ...
    /**
     * 各迁移模块的写入计数（模块名 => 条数）。run() 末尾逐行输出，供迁移验收核对总数；
     * 初始为空，任何模块漏写计数都会在验收时表现为缺失行。
     *
    ...
    /**
     * 旧库邮箱出现次数映射（小写 trim 后的 email => 次数）。用于旧 agents/user 两表
     * 联合去重判断：次数 >1 的邮箱生成账号时必须改写，避免撞 user_logins.email 唯一索引。
     *
     * @var array<string, int>
     */
    private $emailCounts = [];
    ...
    /**
     * 已分配的邮箱集合。与 $emailCounts 配合保证整个迁移过程邮箱唯一、不重复注册；
     * 缺失时并发迁移会插出重复登录账号。
     *
    ...
    /**
     * 旧组别 ID => 新 group_configs.id 映射。交易与用户迁移按它把旧 group 字段落到新库；
     * 映射缺失会把用户迁到错误交易组，直接影响 MT4 成交与佣金归属。
     *
     * @var array<int, int>
     */
    private $groupMap = [];
    ...
    /**
     * 旧等级 ID => 新 agent_levels.id 映射。用户迁移时按它还原代理等级；
     * 缺失会破坏层级统计与佣金比例的取数前提。
     *
     * @var array<int, int>
     */
    ...
    /**
     * 旧用户 ID => 账号类型（1=代理，2=客户）。迁移与层级构建都以它区分身份，
     * 错分类会把客户挂成代理或反之，污染整棵代理树。
     *
     * @var array<int, int>
     */
    ...
    /**
     * 旧用户 ID => 新库 parent_id（上级代理用户 ID）映射。构建 agent_descendants 闭包的输入；
     * 映射错误会让关系码解析与数据范围校验在新库失效。
     *
     * @var array<int, int>
     */
    private $parentMap = [];
    ...
    /**
     * 新用户 ID => 家谱链（family_tree 文本）映射。层级迁移按它批量写入 user_infos.family_tree，
     * 保证闭包关系与家谱字段一致；缺失会导致代理树可见范围计算错误。
     *
     * @var array<int, string>
     */
    private $familyTrees = [];
    ...
    /**
     * 旧用户 ID => 原始用户名映射。落库与日志输出共用，保持新旧身份可对照追溯；
     * 迁移后排查数据问题时依赖该映射回查旧库记录。
     *
     * @var array<int, string>
     */
    private $userNames = [];
    ...
    private function loadReferenceMaps(): void
    {
        ...
        // legacy_group_id 对应旧 user/agents.group_id；pair_id 只表示当前表配对关系，不能反向充当身份键。
        $this->groupMap = DB::table('group_configs')
            ->whereNotNull('legacy_group_id')
            ->pluck('id', 'legacy_group_id')
            ->map(function ($id) {
                return (int) $id;
            })
            ->all();
    ...
    private function updateIdSequences(): void
    {
        ...
        DB::table('id_sequences')->updateOrInsert(
            ['type' => 'agent'],
            ['current_value' => $maxAgentId + 1, 'prefix' => '', 'step' => 1, 'updated_at' => $now, 'created_at' => $now]
        );
        DB::table('id_sequences')->updateOrInsert(
            ['type' => 'customer'],
            ['current_value' => $maxCustomerId + 1, 'prefix' => '', 'step' => 1, 'updated_at' => $now, 'created_at' => $now]
        );
    }
     ...
     *
     * 参数逻辑说明：
     * - user_id=1001 同时作为业务编号和已完成开户的 MT4 编号。
     * - agent@test.com / abc123 仅用于本地演示与闭环验收。
     *
     * @return void 写入成功不返回值；数据库约束失败时抛出异常。
     */
    private function restoreFrontTestAgent(): void
    {
        ...
        DB::table('user_infos')->updateOrInsert(
            ...
            [
                ...
                // 已同步测试代理必须满足新版登录的不变量，MT4 编号不能保留未开户占位值 0。
                'mt4_code' => $userId,
                'trading_mode' => 0,
                'settle_method' => 1,
                'settle_cycle' => 1,
    ...
    {
        // 旧库密码格式不可验证且不再继承，所有迁移账号统一使用需求指定的初始密码。
        return Hash::make('abc123');
    }
