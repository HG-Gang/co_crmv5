...
<div class="flex items-center justify-between mb-8">
    <div>
        <div class="flex items-center gap-3 mb-2">
            <a href="{{ route('admin_tailwind_page_deposits') }}" class="text-slate-600 hover:text-slate-800">
                <i class="fas fa-arrow-left"></i> 返回入金管理
            </a>
        </div>
        <h1 class="text-3xl font-bold text-slate-800">入金批量导入</h1>
        <p class="text-slate-600 mt-2">通过Excel文件批量导入入金记录</p>
    </div>
    <div class="flex gap-3">
        <a href="{{ route('admin_api_deposit_imports_template') }}" class="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold rounded-lg hover:shadow-lg transition">
...
<div class="bg-white rounded-xl shadow-lg p-8 mb-6">
    <div class="max-w-3xl mx-auto">
        <div class="text-center mb-6">
            ...
            <h2 class="text-2xl font-bold text-slate-800 mb-2">上传入金数据文件</h2>
            <p class="text-slate-600">支持 .xlsx 和 .csv 格式，单次最多导入 1000 条记录</p>
        </div>

        <div id="uploadArea" class="border-2 border-dashed border-slate-300 rounded-xl p-12 text-center hover:border-blue-500 transition cursor-pointer" onclick="document.getElementById('fileInput').click()">
        ...
        <div id="progressContainer" class="hidden mt-6">
            <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-sm font-semibold text-blue-600" id="progressText">0%</span>
            </div>
            <div class="w-full bg-slate-200 rounded-full h-3 overflow-hidden">
                <div id="progressBar" class="h-full bg-gradient-to-r from-blue-500 to-indigo-600 transition-all duration-300" style="width: 0%"></div>
            </div>
        </div>
...
function uploadFile() {
    ...
    fetch('{{ route("admin_api_deposit_imports_upload") }}', {
        method: 'POST',
        headers: {
...
function loadImportHistory(page = 1) {
    ...
    fetch(`{{ route('admin_api_deposit_imports_history') }}?page=${page}`, {
...
function renderHistoryTable(imports) {
    ...
    tbody.innerHTML = imports.map(imp => `
        <tr class="hover:bg-slate-50 transition">
            ...
            <td class="px-6 py-4">
                <button onclick="viewImportDetail(${imp.id})" class="px-3 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded hover:bg-blue-200 transition">
...
function renderPagination(pagination) {
    ...
    for (let i = Math.max(1, currentPage - 2); i <= Math.min(totalPages, currentPage + 2); i++) {
        html += `<button onclick="loadImportHistory(${i})" class="px-3 py-1 border ${i === currentPage ? 'bg-blue-500 text-white' : 'border-slate-300 hover:bg-slate-50'} rounded">${i}</button>`;
    }
...
function viewImportDetail(id) {
    fetch(`{{ route('admin_api_deposit_imports_detail', ['id' => '__ID__']) }}`.replace('__ID__', id), {
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
    })
    .then(res => res.json())
    .then(data => {
                    ...
                    <div class="border-t border-slate-200 pt-4">
                        ...
                        <div class="grid grid-cols-4 gap-4">
                            ...
                            <div class="text-center p-4 bg-blue-50 rounded-lg">
                                <p class="text-sm text-blue-600 mb-1">状态</p>
