<?php

/**
 Created by PhpStorm.
 * Project name co_crmv5.
 * User: Huang Gang
 * Date: 2026/07/05
 * Time: 16:42
 */

use Illuminate\Database\Migrations\Migration;
use Illuminate\Support\Facades\DB;

/**
 * 新增后台产品/交易品种权限。
 *
 * 文件功能：
 * - 产品/交易品种页面读取 `symbol_prices` 配置，并汇总 `mt4_trades` 当前持仓数据，是后台交易和风控链路的基础只读页面。
 * - 页面权限通过 permissions.route=/admin/productions 控制菜单可见性。
 * - API 权限通过 permissions.api_route=admin_api_productionList 交给 check.permission:admin 做后端鉴权。
 * - 本迁移只维护权限字典，不直接写入 role_permissions，具体角色是否可见仍由后台权限分配界面决定。
 */
class AddAdminProductionPermissions extends Migration
{
    /**
     * 执行迁移：写入产品/交易品种页面权限和列表 API 权限。
     *
     * @return void
     */
    public function up()
    {
        $pageId = $this->upsertPermission([
            'name' => '产品/交易品种',
            'slug' => 'admin_productions',
            'parent_id' => 0,
            'type' => 1,
            'icon' => 'layui-icon-chart',
            'sort' => 380,
            'route' => '/admin/productions',
            'api_route' => '',
        ]);

        $this->upsertPermission([
            'name' => '查看产品/交易品种',
            'slug' => 'admin_production_list',
            'parent_id' => $pageId,
            'type' => 3,
            'icon' => '',
            'sort' => 10,
            'route' => '',
            'api_route' => 'admin_api_productionList',
        ]);

        $this->upsertPermission([
            'name' => '新增产品/交易品种',
            'slug' => 'admin_production_create',
            'parent_id' => $pageId,
            'type' => 3,
            'icon' => '',
            'sort' => 20,
            'route' => '',
            'api_route' => 'admin_api_createProduction',
        ]);

        $this->upsertPermission([
            'name' => '编辑产品/交易品种',
            'slug' => 'admin_production_update',
            'parent_id' => $pageId,
            'type' => 3,
            'icon' => '',
            'sort' => 30,
            'route' => '',
            'api_route' => 'admin_api_updateProduction',
        ]);

        $this->upsertPermission([
            'name' => '删除产品/交易品种',
            'slug' => 'admin_production_delete',
            'parent_id' => $pageId,
            'type' => 3,
            'icon' => '',
            'sort' => 40,
            'route' => '',
            'api_route' => 'admin_api_deleteProduction',
        ]);

        $this->upsertPermission([
            'name' => '导出产品/交易品种',
            'slug' => 'admin_production_export',
            'parent_id' => $pageId,
            'type' => 3,
            'icon' => '',
            'sort' => 50,
            'route' => '',
            'api_route' => 'admin_api_exportProductions',
        ]);
    }

    /**
     * 回滚迁移：删除本迁移维护的产品/交易品种权限节点。
     *
     * @return void
     */
    public function down()
    {
        DB::table('permissions')->whereIn('slug', [
            'admin_productions',
            'admin_production_list',
            'admin_production_create',
            'admin_production_update',
            'admin_production_delete',
            'admin_production_export',
        ])->delete();
    }

    /**
     * 写入或更新单条权限配置。
     *
     * 参数逻辑说明：
     * - slug：权限唯一标识，页面、按钮和角色授权都使用这个稳定 key。
     * - parent_id：父权限 ID，页面节点为 0，列表动作挂在页面节点下。
     * - type：权限类型，1=菜单/页面，3=按钮/API 动作。
     * - route：Blade 页面路径，只给页面权限填写。
     * - api_route：Laravel 命名 API 路由，只给动作权限填写。
     *
     * @param array<string, mixed> $permission 权限配置数组。
     * @return int permissions.id，用于绑定子权限 parent_id。
     */
    private function upsertPermission(array $permission)
    {
        $now = now()->format('Y-m-d H:i:s');

        DB::table('permissions')->updateOrInsert(
            ['slug' => $permission['slug']],
            [
                'name' => $permission['name'],
                'guard_type' => 'admin',
                'parent_id' => $permission['parent_id'],
                'type' => $permission['type'],
                'icon' => $permission['icon'],
                'sort' => $permission['sort'],
                'route' => $permission['route'],
                'api_route' => $permission['api_route'],
                'status' => 1,
                'created_at' => $now,
                'updated_at' => $now,
            ]
        );

        return (int) DB::table('permissions')->where('slug', $permission['slug'])->value('id');
    }
}
