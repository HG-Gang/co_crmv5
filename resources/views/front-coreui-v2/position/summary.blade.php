...
<div class="container-lg px-4">
    ...
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="mb-2">持仓汇总</h2>
            <p class="text-body-secondary mb-0">查看所有MT4账户的持仓统计信息</p>
        </div>
    </div>
            ...
            <div class="card shadow-sm border-0 h-100">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            <p class="text-body-secondary mb-2">总持仓手数</p>
        ...
        <div class="col-lg-3 col-md-6">
            <div class="card shadow-sm border-0 h-100">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            <p class="text-body-secondary mb-2">持仓盈亏</p>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-transparent border-bottom">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0">
                    <i class="cil-layers me-2"></i>持仓明细
                </h5>
...
function loadSummary() {
    ...
    fetch('{{ route("front_api_position_summary") }}?' + params.toString(), {
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
    })
    .then(res => res.json())
    .then(data => {
...
function loadPositions(page = 1) {
    ...
    fetch('{{ route("front_api_position_list") }}?' + params.toString(), {
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
    })
    .then(res => res.json())
    .then(data => {
...
function renderPositions(positions) {
    ...
    const html = positions.map(p => `
        <tr>
            ...
            <td class="text-body-secondary">${p.order_number || '-'}</td>
            ...
            <td class="text-body-secondary small">${formatPrice(p.stop_loss)}</td>
            <td class="text-body-secondary small">${formatPrice(p.take_profit)}</td>
            <td class="${p.profit >= 0 ? 'text-success' : 'text-danger'} fw-bold">
                ${p.profit >= 0 ? '+' : ''}${formatCurrency(p.profit)}
            </td>
            <td>${formatCurrency(p.margin)}</td>
            <td>
                <a href="{{ route('front_coreui_v2_page_position_summary_detail') }}?order=${p.order_number}" class="btn btn-sm btn-outline-primary">
                    详情
                </a>
            </td>
        </tr>
