 ...
 *
 * 文件功能：
 * - 验证 GET /api/front/commissions/realtime 在 agent_descendants 表无数据
 *   （family_tree 缺失）时，降级使用 user_infos.parent_id 树构建订单链 order_chain。
 * - 验证该降级修复已登记在 docs/admin-backend-blade-permission-final-checklist.md（第 179 项）。
 *
 * 适用场景：
 * - 回归 CommissionController::orderChain 与 FrontLegacyData::userScopeIds 的
 *   parent_id 降级路径，防止订单链重新依赖可能缺行的 agent_descendants 数据。
 *
 * 入参例子：
 * - 构造 root(411790100) -> sub(411790101) -> customer(411790102) 三级 parent_id 链，
 *   由 root 代理登录后请求 /api/front/commissions/realtime?orderId={ticket}&per_page=5，
...
class FrontCommissionOrderChainScopeFallbackModuleTest extends TestCase
{
    use DatabaseTransactions;

    public function test_realtime_commission_uses_parent_id_tree_for_order_chain_when_family_tree_rows_are_missing(): void
    {
        $rootAgentId = 411790100;
        ...
        $this->insertUserInfo($rootAgentId, 'commission-chain-root-agent', 1, 0);
        $this->insertUserInfo($subAgentId, 'commission-chain-sub-agent', 1, $rootAgentId);
        $this->insertUserInfo($customerId, 'commission-chain-customer', 2, $subAgentId);
        ...
        $response = $this->withoutMiddleware([JwtAuthMiddleware::class, SingleSignOn::class])
            ->actingAs($login, 'user')
            ->getJson('/api/front/commissions/realtime?orderId=' . $ticket . '&per_page=5');
        ...
        $this->assertSame('commission-chain-root-agent', $chain[0]['user_name']);
        $this->assertSame('commission-chain-sub-agent', $chain[1]['user_name']);
        $this->assertSame('commission-chain-customer', $chain[2]['user_name']);
    }

    public function test_final_checklist_records_commission_order_chain_scope_fallback(): void
    {
        $checklist = file_get_contents(base_path('docs/admin-backend-blade-permission-final-checklist.md'));

        $this->assertStringContainsString('## 179.', $checklist);
        $this->assertStringContainsString('CommissionController::orderChain', $checklist);
        $this->assertStringContainsString('FrontCommissionOrderChainScopeFallbackModuleTest', $checklist);
    ...
    private function insertUserInfo(int $userId, string $userName, int $accountType, int $parentId): void
    {
        ...
        $loginId = DB::table('user_logins')->insertGetId([
            'user_id' => $userId,
            'email' => 'front-commission-chain-' . $userId . '@example.test',
        ...
        DB::table('user_infos')->insert([
            ...
            'login_id' => $loginId,
            'user_name' => $userName,
            'phone' => '1787900' . substr((string) $userId, -4),
            'gender' => 1,
            'account_type' => $accountType,
            'parent_id' => $parentId,
    ...
    private function insertClosedTrade(int $userId, int $ticket): void
    {
        ...
        DB::table('user_trades')->insert([
            ...
            'profit' => 100,
            'taxes' => 0,
            'comment' => 'front commission order chain scope fallback test',
            'internal_id' => 0,
            'margin_rate' => 1,
            'timestamp_val' => $now,
            'magic' => 0,
