 ...
 *
 * 文件功能：
 * - 断言 admin_real_commission_business_2026_08_21 证据组覆盖全部三条方法级路由。
 * - 每条路由必须 verified、绑定正确当前路由名，且七个核验维度全部 passed。
 * - 每个维度的证据文本必须包含对应路由键，保证方法级说明不共享混写。
 * - 全局 summary 必须从 475 行重算且 verified 不低于本批次下限。
 */
final class AdminRealCommissionBusinessMatrixClosureTest extends TestCase
{
    /**
     * 实时返佣证据组的键名。按它从核验矩阵 JSON（storage/app/audits/旧项目模块逻辑迁移核验矩阵.json）
     * 筛选本模块行并断言每行 verification_group 与之一致；值不匹配即门禁失败，防止证据混组。
     */
    private const VERIFICATION_GROUP = 'admin_real_commission_business_2026_08_21';
    ...
    /**
     * 实时返佣模块的旧路由认领清单（三条）：键为「HTTP 方法 旧URI」，值为矩阵认定的当前路由名。
     * 门禁逐条断言 verified、绑定与七维独立出证；路由增删必须同步维护，否则漏出证或误报。
     */
    private const EXPECTED_ROUTES = [
        'POST index/admin/order/realCommissionListSearch' => 'legacy_admin_2f653bf34f32b481',
        'POST index/admin/order/realCommissionListSearchV2' => 'legacy_admin_7b1aa464cd2a9baa',
        'GET index/admin/order/real_commission_list' => 'legacy_admin_675c6893d740a8a8',
    ];

    public function test_all_three_real_commission_routes_have_independent_complete_evidence(): void
    {
    ...
    public function test_global_matrix_summary_reflects_the_real_commission_closure(): void
    {
        $matrix = $this->readMatrix();
        $counts = [
            'verified' => 0,
        ...
        $this->assertGreaterThanOrEqual(459, $counts['verified']);
    }
