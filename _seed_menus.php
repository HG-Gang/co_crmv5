 ...
 *
 * 文件功能：
 * - 引导 Laravel 容器后写入后台菜单与权限绑定数据，保证后台侧边栏与
 *   权限点完整可用。
 *
 * 适用场景：
 * - 由测试基础设施调用，普通业务无需执行。
...
// ===== FRONT GUARD =====
$insert(['仪表盘','front_dashboard','front',0,2,'fas fa-tachometer-alt',1,'/front/dashboard','front_api_dashboard']); // 48

$fp = $insert(['个人中心','front_profile','front',0,1,'fas fa-user',2,'','']); // 49
$insert(['个人资料','front_profile_info','front',$fp,2,'fas fa-id-card',1,'/front/profile','front_api_profile']); // 50
$insert(['编辑资料','front_profile_edit','front',$fp,2,'fas fa-user-edit',2,'/front/profile/edit','front_api_profile_update']); // 51
$insert(['修改密码','front_change_pwd','front',$fp,2,'fas fa-key',3,'/front/profile/change-password','front_api_profile_password']); // 52
$insert(['修改邮箱','front_change_email','front',$fp,2,'fas fa-envelope',4,'/front/profile/change-email','front_api_profile_email']); // 53

$fa = $insert(['账户管理','front_account','front',0,1,'fas fa-wallet',3,'','']); // 54
$insert(['账户信息','front_account_info','front',$fa,2,'fas fa-info-circle',1,'/front/account/info','front_api_account_profile']); // 55
$insert(['账户余额','front_account_balance','front',$fa,2,'fas fa-coins',2,'/front/account/balance','front_api_account_balance']); // 56
$insert(['提交凭证','front_voucher','front',$fa,2,'fas fa-receipt',3,'/front/account/voucher','front_api_account_vouchers']); // 57
$insert(['注销账户','front_cancel','front',$fa,2,'fas fa-user-times',4,'/front/account/cancel','front_api_account_cancellation']); // 58

$fd = $insert(['入出金','front_deposit_withdraw','front',0,1,'fas fa-dollar-sign',4,'','']); // 59
$insert(['入金','front_deposit','front',$fd,2,'fas fa-plus-circle',1,'/front/deposit','front_api_deposits_history']); // 60
$insert(['出金','front_withdraw','front',$fd,2,'fas fa-minus-circle',2,'/front/withdraw','front_api_withdrawals_history']); // 61
$insert(['账户流水','front_flow','front',$fd,2,'fas fa-stream',3,'/front/flow','front_api_flows_account']); // 62

$ft = $insert(['交易记录','front_trading','front',0,1,'fas fa-chart-bar',5,'','']); // 63
$insert(['仓位总结','front_position_summary','front',$ft,2,'fas fa-chart-pie',1,'/front/position/summary','front_api_positions_summary']); // 64
$insert(['持仓订单','front_open_orders','front',$ft,2,'fas fa-play-circle',2,'/front/order/open','front_api_orders_open']); // 65
$insert(['历史订单','front_closed_orders','front',$ft,2,'fas fa-history',3,'/front/order/closed','front_api_orders_closed']); // 66

$fag = $insert(['代理管理','front_agent','front',0,1,'fas fa-sitemap',6,'','']); // 67
$insert(['下级代理','front_agent_sub','front',$fag,2,'fas fa-user-friends',1,'/front/agent/sub','front_api_agents_direct']); // 68
$insert(['直属客户','front_agent_customers','front',$fag,2,'fas fa-users',2,'/front/agent/customers','front_api_agents_direct_customers']); // 69
$insert(['代理级别确认','front_agent_confirm','front',$fag,2,'fas fa-check-circle',3,'/front/agent/confirm-level','front_api_agents_level_confirmation']); // 70
$insert(['组别变更','front_group_change','front',$fag,2,'fas fa-exchange-alt',4,'/front/agent/group-change','front_api_agents_group_changes']); // 71

$fc = $insert(['返佣管理','front_commission','front',0,1,'fas fa-money-bill-wave',7,'','']); // 72
$insert(['实时返佣','front_commission_rt','front',$fc,2,'fas fa-bolt',1,'/front/commission/realtime','front_api_commissions_realtime']); // 73
$insert(['返佣历史','front_commission_hist','front',$fc,2,'fas fa-history',2,'/front/commission/history','front_api_commissions_history']); // 74
$insert(['佣金转账','front_commission_transfer','front',$fc,2,'fas fa-paper-plane',3,'/front/commission/transfer','front_api_commissions_history']); // 75

$fg = $insert(['礼品中心','front_gift','front',0,1,'fas fa-gift',8,'','']); // 76
$insert(['地址管理','front_gift_address','front',$fg,2,'fas fa-map-marker-alt',1,'/front/gift/address','front_api_gift_addresses_index']); // 77
$insert(['礼品列表','front_gift_list','front',$fg,2,'fas fa-box',2,'/front/gift/list','front_api_gifts']); // 78
